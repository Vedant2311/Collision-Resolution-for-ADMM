        ...
        if(self.con1(x) >= 0 and self.con2(x) >= 0):
            ...
            for i in range(0,len(x),3):
                ...
                if(self.con1([x1,y1,z1]) < 0):
                    ...
                    h = 1e-10
                    surface_normal_x = self.con1(collision_point) - self.con1(collision_point + np.array([h,0,0]))
                    surface_normal_y = self.con1(collision_point) - self.con1(collision_point + np.array([0,h,0]))
                    surface_normal_z = self.con1(collision_point) - self.con1(collision_point + np.array([0,0,h]))
                    ...
                    x2_perp = np.dot((x2-collision_point),surface_normal) * surface_normal
                    x2_tang = x2 - collision_point - x2_perp
                    final_x = collision_point  + 1*(- x2_perp + x2_tang)
                    
                    x1_perp = np.dot((collision_point - x1),surface_normal) * surface_normal
                    x1_tang = collision_point - x1 - x1_perp
                    new_cur_pos = collision_point + x1_perp + x1_tang
                ...
                if(self.con2([x1,y1,z1]) < 0):
                    ...
                    h = 1e-10
                    surface_normal_x = self.con2(collision_point) - self.con2(collision_point + np.array([h,0,0]))
                    surface_normal_y = self.con2(collision_point) - self.con2(collision_point + np.array([0,h,0]))
                    surface_normal_z = self.con2(collision_point) - self.con2(collision_point + np.array([0,0,h]))
                    surface_normal = np.array([surface_normal_x,surface_normal_y,surface_normal_z])/h
...
frame_file = open(os.path.join(__location__, 'data/obstacle/frame_inelastic_1'), 'ab')
  
# source, destination 
pickle.dump(frame, frame_file)                      
frame_file.close() 

mesh_file = open(os.path.join(__location__, 'data/obstacle/mesh_inelastic_1'), 'ab') 
...
def main():
    ...
    glLight(GL_LIGHT0, GL_POSITION,  (1000,1000,1000, 1))
    ...
    if not os.path.exists(os.path.join(__location__,"images/inelastic_1/")):
        os.makedirs(os.path.join(__location__,"images/inelastic_1/"))
    while (True):
        ...
        #pg.image.save(window, os.path.join(__location__,"images/inelastic_1/image" + str(img_id) + ".png"))
        img_id+=1
        pg.display.flip()
        pg.time.wait(1)
