...
frame_file = open(os.path.join(__location__, 'data/one_clip_mesh_scipy/frame'), 'ab')
  
# source, destination 
pickle.dump(frame, frame_file)                      
frame_file.close() 

mesh_file = open(os.path.join(__location__, 'data/one_clip_mesh_scipy/mesh'), 'ab') 
...
def main():
    ...
    glRotate(-70, 0,5,0)
    glLight(GL_LIGHT0, GL_POSITION,  (1000,1000,0, 1))
    ...
    if not os.path.exists(os.path.join(__location__,"images/one_clip/")):
        os.makedirs(os.path.join(__location__,"images/one_clip/"))
    while (True):
        ...
        #pg.image.save(window, os.path.join(__location__,"images/one_clip/image" + str(img_id) + ".png"))
        img_id+=1
        pg.display.flip()
        pg.time.wait(1)
